...
with tab1:
    ...
    # ── ML Baseline Zone Prediction ──────────────────────────────────────────────
    # Trains a Random Forest on the ORIGINAL baseline SKU table (fixed reference).
    # Intent: ML = what the baseline data says zones should be.
    #         Optimization Engine = what the EDITED data says zones should be.
    #         Comparison reveals drift between baseline expectations and new inputs.
    from sklearn.ensemble import RandomForestClassifier

    BASELINE_DF = get_sku_data()   # always the unedited original

    def train_ml_model(baseline: pd.DataFrame):
        """Train RF on baseline SKU features → predict Opt_Zone (A/B/C)."""
        features = ["Monthly_Hits", "Avg_Units", "Unit_Weight_lbs"]
        labels   = baseline["Monthly_Hits"].apply(classify)   # use same classify fn
        clf = RandomForestClassifier(n_estimators=100, random_state=42)
        clf.fit(baseline[features], labels)
        return clf

    if "ml_model" not in st.session_state:
        st.session_state.ml_model = train_ml_model(BASELINE_DF)
    if "ml_predictions" not in st.session_state:
        st.session_state.ml_predictions = None
    if "show_ml_compare" not in st.session_state:
        st.session_state.show_ml_compare = False

    st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
    st.markdown('<div class="sec-hdr">🧠 ML Zone Prediction — Baseline Random Forest Model</div>',
                unsafe_allow_html=True)

    st.markdown(
        f'<div class="info-box" style="margin-bottom:10px;font-size:0.78rem;">'
        f'<b style="color:{BLUE};">How this works:</b> The ML model is trained on the '
        f'<b>original baseline SKU table</b> (fixed reference point). '
        f'When you edit hit frequencies and run the Optimization Engine, '
        f'you can compare the engine\'s new output against the ML\'s baseline prediction '
        f'to see which SKUs drifted outside what the model expected. '
        f'<b>Agreement = stable signal · Mismatch = demand shift detected.</b></div>',
        unsafe_allow_html=True)

    ml_btn_col, _ = st.columns([1, 3])
    with ml_btn_col:
        if st.button("🧠  RUN ML PREDICTION", key="run_ml"):
            live_src = st.session_state.edited_df.copy()
            features = ["Monthly_Hits", "Avg_Units", "Unit_Weight_lbs"]
            preds    = st.session_state.ml_model.predict(live_src[features])
            proba    = st.session_state.ml_model.predict_proba(live_src[features])
            classes  = st.session_state.ml_model.classes_
            conf     = [round(float(max(p)), 2) for p in proba]
            ml_df    = live_src[["SKU", "Description", "Monthly_Hits", "Current_Zone"]].copy()
            ml_df["ML_Predicted_Zone"] = preds
            ml_df["ML_Confidence"]     = conf
            st.session_state.ml_predictions  = ml_df
            st.session_state.show_ml_compare = False   # reset comparison until engine runs

    # Show ML prediction cards if they exist
    if st.session_state.ml_predictions is not None:
        ml_df = st.session_state.ml_predictions
        mc1, mc2, mc3 = st.columns(3)
        for col_m, zone, clr in zip([mc1, mc2, mc3], ["A", "B", "C"], [GREEN, WARN, RED]):
            ct = len(ml_df[ml_df["ML_Predicted_Zone"] == zone])
            with col_m:
                lbl = {"A":"GOLDEN ZONE (A)", "B":"MID ZONE (B)", "C":"COLD ZONE (C)"}[zone]
                st.markdown(
                    f'<div class="mini-card" style="border-top:3px solid {clr};">'
                    f'<div class="mini-lbl">{lbl}</div>'
                    f'<div class="mini-val" style="color:{clr};">{ct} SKUs</div>'
                    f'<div style="font-size:0.68rem;color:{GREY_TXT};margin-top:2px;">'
                    f'ML baseline prediction</div></div>',
                    unsafe_allow_html=True)

        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        ml_rows = []
        for _, row in ml_df.iterrows():
            z = row["ML_Predicted_Zone"]
            clr = GREEN if z=="A" else (WARN if z=="B" else RED)
            bg  = "#E8F8F1" if z=="A" else ("#FFF4E5" if z=="B" else "#FEF0ED")
            conf_pct = int(row["ML_Confidence"]*100)
            ml_rows.append(
                f'<div style="background:{bg};border-left:3px solid {clr};'
                f'border-radius:6px;padding:8px 12px;margin-bottom:5px;'
                f'display:flex;justify-content:space-between;align-items:center;">'
                f'<div>'
                f'<b style="color:{INK};font-size:0.85rem;">{row["SKU"]}</b>'
                f'<span style="color:{GREY_TXT};font-size:0.75rem;margin-left:8px;">'
                f'{str(row["Description"])[:30]}</span>'
                f'</div>'
                f'<div style="display:flex;gap:12px;align-items:center;">'
                f'<span style="font-size:0.72rem;color:{GREY_TXT};">Current: '
                f'<b style="color:{INK};">{row["Current_Zone"]}</b></span>'
                f'<span style="background:{clr};color:#FFF;font-size:0.72rem;'
                f'font-weight:700;padding:2px 10px;border-radius:10px;">'
                f'→ Zone {z}</span>'
                f'<span style="font-size:0.7rem;color:{GREY_TXT};">conf: '
                f'<b style="color:{INK};">{conf_pct}%</b></span>'
                f'</div></div>'
            )
        st.markdown("".join(ml_rows), unsafe_allow_html=True)
        st.markdown(
            f'<div style="font-size:0.72rem;color:{GREY_TXT};margin-top:4px;">'
            f'⚙️ Trained on baseline SKU table · Random Forest (100 trees) · '
            f'Features: Monthly Hits, Avg Units, Unit Weight · '
            f'<b style="color:{BLUE};">Run Slotting Optimization to compare engine output vs ML prediction.</b>'
            f'</div>', unsafe_allow_html=True)

    # ── ML vs Engine Comparison (shown after both have run) ────────────────────
    if st.session_state.optimized and st.session_state.ml_predictions is not None:
        st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
        st.markdown('<div class="sec-hdr">🔬 ML Prediction vs Engine Output — Validation Report</div>',
                    unsafe_allow_html=True)

        eng_df = st.session_state.opt_df[["SKU", "Opt_Zone", "Monthly_Hits"]].copy()
        ml_p   = st.session_state.ml_predictions[["SKU", "ML_Predicted_Zone", "ML_Confidence"]].copy()
        cmp    = eng_df.merge(ml_p, on="SKU")
        cmp["Match"] = cmp["Opt_Zone"] == cmp["ML_Predicted_Zone"]

        match_ct   = int(cmp["Match"].sum())
        mismatch_ct = int((~cmp["Match"]).sum())
        accuracy   = round(match_ct / len(cmp) * 100, 1)
        acc_clr    = GREEN if accuracy >= 80 else (WARN if accuracy >= 60 else RED)

        va1, va2, va3 = st.columns(3)
        for col_v, (lbl, val, clr, sub) in zip([va1, va2, va3], [
            ("AGREEMENT RATE",   f"{accuracy}%",      acc_clr, f"{match_ct}/{len(cmp)} SKUs match"),
            ("MATCHES",          str(match_ct),        GREEN,   "Engine = ML prediction"),
            ("MISMATCHES",       str(mismatch_ct),     RED if mismatch_ct > 0 else GREEN,
                                                               "Demand shift detected" if mismatch_ct > 0 else "All clear"),
        ]):
            with col_v:
                st.markdown(
                    f'<div class="mini-card" style="border-top:3px solid {clr};">'
                    f'<div class="mini-lbl">{lbl}</div>'
                    f'<div class="mini-val" style="color:{clr};font-size:1.4rem;">{val}</div>'
                    f'<div style="font-size:0.7rem;color:{GREY_TXT};margin-top:2px;">{sub}</div>'
                    f'</div>', unsafe_allow_html=True)
        ...
        for _, row in cmp.iterrows():
            match  = row["Match"]
            bg     = "#E8F8F1" if match else "#FEF0ED"
            bdr    = GREEN if match else RED
            icon   = "✅" if match else "⚠️"
            status = "MATCH" if match else "MISMATCH"
            sclr   = GREEN if match else RED
            ez     = row["Opt_Zone"]
            mz     = row["ML_Predicted_Zone"]
            ezclr  = GREEN if ez=="A" else (WARN if ez=="B" else RED)
            mzclr  = GREEN if mz=="A" else (WARN if mz=="B" else RED)
            conf_pct = int(row["ML_Confidence"]*100)
            note   = ("Demand change confirmed by engine" if not match
                      else "Stable — baseline model and live data agree")
            st.markdown(
                f'<div style="background:{bg};border-left:4px solid {bdr};'
                f'border-radius:6px;padding:10px 14px;margin-bottom:6px;">'
                f'<div style="display:flex;justify-content:space-between;align-items:center;">'
                f'<div>'
                f'<b style="color:{INK};font-size:0.88rem;">{icon} {row["SKU"]}</b>'
                f'<span style="font-size:0.7rem;color:{GREY_TXT};margin-left:8px;">'
                f'Hits/Mo: {row["Monthly_Hits"]}</span>'
                f'</div>'
                f'<span style="background:{sclr};color:#FFF;font-size:0.68rem;'
                f'font-weight:700;padding:2px 10px;border-radius:10px;">{status}</span>'
                f'</div>'
                f'<div style="display:flex;gap:20px;margin-top:6px;font-size:0.78rem;">'
                f'<span>ML Baseline: <b style="background:{mzclr};color:#FFF;'
                f'padding:1px 8px;border-radius:8px;">Zone {mz}</b> '
                f'<span style="color:{GREY_TXT};">({conf_pct}% conf)</span></span>'
                f'<span>Engine Output: <b style="background:{ezclr};color:#FFF;'
                f'padding:1px 8px;border-radius:8px;">Zone {ez}</b></span>'
                f'<span style="color:{GREY_TXT};font-size:0.72rem;font-style:italic;">{note}</span>'
                f'</div></div>',
                unsafe_allow_html=True)

        if mismatch_ct > 0:
            st.markdown(
                f'<div style="background:#FFF4E5;border:1px solid {WARN};border-radius:8px;'
                f'padding:0.8rem 1rem;margin-top:8px;font-size:0.8rem;color:{INK};">'
                f'<b style="color:{WARN};">⚠️ Demand Shift Detected:</b> {mismatch_ct} SKU(s) '
                f'moved to a different zone than the ML baseline predicted. '
                f'This confirms your hit-frequency edits created real reclassification — '
                f'the engine is correct, and the ML model reflects the <i>old</i> demand pattern. '
                f'In production, this gap triggers a model retraining cycle.</div>',
                unsafe_allow_html=True)
        else:
            st.markdown(
                f'<div style="background:#E8F8F1;border:1px solid {GREEN};border-radius:8px;'
                f'padding:0.8rem 1rem;margin-top:8px;font-size:0.8rem;color:{INK};">'
                f'<b style="color:{GREEN};">✅ Full Agreement:</b> ML baseline and engine output '
                f'agree on all {len(cmp)} SKUs. No demand drift detected — current slotting '
                f'strategy is validated by both models.</div>',
                unsafe_allow_html=True)

    # ── ML Risk Prediction — full width, shown only before optimization ─────────
    if not st.session_state.optimized:
        st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
        st.markdown('<div class="sec-hdr">📊 SKU Risk Scoring — 30-Day Inventory and Placement Risk</div>', unsafe_allow_html=True)
